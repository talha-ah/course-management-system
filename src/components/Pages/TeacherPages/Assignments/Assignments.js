...
class Assignments extends Component {
  ...
  onDeleteAssignment = async (itemId) => {
    try {
      this.setState({ isLoading: true });
      const res = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/teacher/deleteassignment/${this.state.assignments._id}/${itemId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: "Bearer " + this.props.token,
                  ...
                  <tr key={row._id} className={classes.onRowHoverEffect}>
                    ...
                    <td>
                      ...
                      <TableButton
                        buttonType="red"
                        onClick={() => {
                          this.onDeleteAssignment(row._id);
                        }}
                      >
                        {"Delete"}
                      </TableButton>
